...
float cfpm(float op1,float op2,int tune)
     {
		 //mbm
    ...
    int op1_frclen, op2_frclen;
    int result_sign, result_exp,result_frclen;
    float result_f, result_e, result_fnew;
    ...
    result_e = op1_exp + op2_exp;
    result_f = op1_f + op2_f;

    // cout << result_e << result_f << endl;

    if (result_f >= 1)
    {
        result_f += 0.078125/2.0;
        result_f = result_f/2.0;
        result_e += 1;
    }
    ...
    // cout << result_e << result_f << "frac_len" << frac_len << endl;
    ...
    if (result_sign == 1)
    {
        result = -1*((result_f)*pow(2.0, result_e));
    }
    else
    {
        result = (result_f)*pow(2.0, result_e);
    }
    return result;
}
