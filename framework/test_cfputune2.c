...
float cfpm(float a,float b,int tune)
    ...
    float e1 = floor(log2(in1)); //Exponent of in1

    // Extracting the Mantissa Bits of the discarded number
    float mant1 = in1/pow(2,e1)-1;

    if (mant1 >= 0.5)
    {
        // count =1;
        e1 = e1 + 1;
        output = sign*pow(2,e1)*in2;
        // isApproxMul = true;
    }
    else if (mant1< 0.5)
    {
