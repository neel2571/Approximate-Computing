...
if __name__ == "__main__":
    ...
    results = open("../results/hsja_resnet18_cifar10_tensor_result.csv", "w")
    ...
    for data in data_loader:
        ...
        adv_imgs = hop_skip_jump_attack(model_fn = model, x = inputs, norm = 2)
        adv_outputs = model(adv_imgs)
        _, adv_predicted = torch.max(adv_outputs.data, 1)
        for i in range(inputs.shape[0]):
            torch.save(adv_imgs[i], f"../tensors/resnet18_cifar10/hsja_{count}_resnet18_cifar10.pt")
            print("Saved!")
